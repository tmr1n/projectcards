		...
		<div className='bg-white rounded-2xl shadow-[0_0_12px_rgba(0,0,0,0.07)] p-4'>
			<div className='flex flex-col md:flex-row gap-3 md:gap-6 md:items-center'>
				...
				<div className='flex-1 flex items-center gap-2 justify-between min-w-0'>
					...
					<IconBtn
						tooltip={t('pronounceTranslation')}
		...
		<div className='bg-white rounded-2xl shadow-[0_0_12px_rgba(0,0,0,0.07)] p-6'>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
				{question.options.map(opt => {
					let style =
						'border border-gray-200 text-gray-800 hover:border-violet-400 hover:bg-violet-50'
					if (!result && selected === opt)
						style = 'border-2 border-violet-500 bg-violet-50 text-violet-800'
					if (result && selected === opt) {
						style =
							result === 'correct'
					...
					if (result === 'wrong' && opt === question.correct)
...
export default function FlashCardPage() {
	...
	const handleShuffle = () => {
		setDisplayCards(
			shuffled ? (deck?.cards ?? []) : shuffle([...(deck?.cards ?? [])])
		)
		setShuffled(v => !v)
		setCardIndex(0)
		setFlipped(false)
										...
										>
											...
											<div className='absolute inset-0 bg-white shadow-[0_0_20px_rgba(0,0,0,0.12)] rounded-lg backface-hidden transform-[rotateY(180deg)]'>
												<div className='flex flex-row items-start justify-end p-4'>
													<IconBtn
														tooltip={t('pronounceTranslation')}
														onClick={() => speak(currentCard.back)}
													>
														<Volume2 />
													</IconBtn>
												</div>
												<p className='flex items-center justify-center text-xl md:text-2xl font-medium select-none text-gray-600 px-6 text-center h-[calc(100%-80px)]'>
													{currentCard.back}
												</p>
											</div>
										</div>
								...
								<div className='flex items-center justify-between mt-6 w-full md:w-187.5'>
									...
									<div className='flex items-center gap-4'>
										...
										<span className='text-gray-800 font-bold py-2 px-4'>
											{cardIndex + 1}/{displayCards.length}
										</span>
										<button
											onClick={() => {
												setCardIndex(i =>
													Math.min(i + 1, displayCards.length - 1)
												)
												setFlipped(false)
											}}
